...
function threaded_progressive_trk_shortest_foremost_topk(tg::temporal_graph,eps::Float64,delta::Float64,k::Int64,verbose_step::Int64,bigint::Bool,algo::String = "trk",diam::Int64 = -1,start_factor::Int64 = 100,sample_step::Int64 = 10,hb::Bool = false)
    ...
    while sampled_so_far <= omega && !stop[1]
        ...
        if (verbose_step > 0 && sampled_so_far % verbose_step == 0)
            ...
            println("P-TRK-SFM (TOP-K). Processed " * string(sampled_so_far) * " pairs in " * finish_partial * " seconds ")
        end
    end
    if stop[1]
