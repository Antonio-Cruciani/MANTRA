function distributed_progressive_wub(tg::temporal_graph,eps::Float64,delta::Float64,verbose_step::Int64,bigint::Bool,algo::String = "trk",diam::Int64 = -1,start_factor::Int64 = 100,sample_step::Int64 = 10,hb::Bool = false)
    ...
    betweenness = @distributed (+) for i in 1:tau
        local_temporal_betweenness = zeros(tg.num_nodes)

        sample = onbra_sample(tg, 1)
        s = sample[1][1]
        z = sample[1][2]
        if algo == "trk"
            _trk_sh_accumulate!(tg,tal,tn_index,bigint,s,z,local_temporal_betweenness)
        elseif algo == "ob"
            _onbra_sh_accumulate!(tg,tal,tn_index,bigint,s,z,local_temporal_betweenness)
        elseif algo == "rtb"
            _sstp_accumulate!(tg,tal,tn_index,s,bigint,local_temporal_betweenness)
        end
        local_temporal_betweenness
    end
    if algo == "rtb"
    ...
    while sampled_so_far < omega && !stop[1]
        betweenness_tmp = @distributed (+) for i in 1:sample_step
            local_temporal_betweenness = zeros(tg.num_nodes)
            sample = onbra_sample(tg, 1)
            s = sample[1][1]
            z = sample[1][2]
