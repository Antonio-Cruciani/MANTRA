...
function threaded_progressive_bernstein(tg::temporal_graph,eps::Float64,delta::Float64,bigint::Bool,algo::String = "trk",vc_upper_bund::Bool = true,diam::Int64 = -1,geo::Float64 = 1.2,start_factor::Int64 = 100,force_gc::Bool = false)
    ...
    @sync for (t, task_range) in enumerate(Iterators.partition(1:tau, task_size))
        Threads.@spawn for _ in @view(vs_active[task_range])
            ...
            z = sample[1][2]
            if algo == "trk"
                _sh_accumulate_trk!(tg,tal,tn_index,bigint,s,z,mc_trials,true,local_temporal_betweenness_bootstrap[t],local_wv[t],mcrade[t],local_sp_lengths[t])
            elseif algo == "ob"
                _sh_accumulate_onbra!(tg,tal,tn_index,bigint,s,z,mc_trials,true,local_temporal_betweenness_bootstrap[t],local_wv[t],mcrade[t],local_sp_lengths[t])
            elseif algo == "rtb"
...
function threaded_progressive_bernstein_topk(tg::temporal_graph,eps::Float64,delta::Float64,k::Int64,bigint::Bool,algo::String = "trk",vc_upper_bund::Bool = true,diam::Int64 = -1,geo::Float64 = 1.2,start_factor::Int64 = 100)
    @assert (algo == "trk") || (algo == "ob") || (algo == "rtb") "Illegal algorithm, use: trk , ob , or rtb"
    start_time = time()
    ntasks = nthreads()
    tal::Array{Array{Tuple{Int64,Int64}}} = temporal_adjacency_list(tg)
    ...
    if !vc_upper_bund 
     ...
     while !sample_stop && !stop[1]
        ...
        if !sample_stop && !stop[1]
            ...
            iteration+=1
            finish_partial = string(round(time() - start_time; digits=4))
            println("P-Bernstein-"*algo*"-SH. Processed " * string(sampled_so_far) * " pairs in " * finish_partial * " seconds | top k converged "*string(overall_converged)*"/"*string(union_sample)*" | Increasing sample size to "*string(next_stopping_samples))
            flush(stdout)
        end
     end
     if stop[1]
...
function threaded_progressive_bernstein_dep(tg::temporal_graph,initial_sample::Int64,epsilon::Float64,delta::Float64,geo::Float64,verbose_step::Int64, bigint::Bool, algo::String = "trk")::Tuple{Array{Float64},Array{Int64},Float64,Float64}
    @assert (algo == "trk") || (algo == "ob") || (algo == "rtb") "Illegal algorithm, use: trk , ob , or rtb"
    start_time = time()
...
function theoretical_error_bound_topk(tilde_b::Array{Float64},tb::Array{Tuple{Int64,Float64}}, sample_size::Int64, eta::Float64,eps::Float64,n::Int64)::Tuple{Bool,Int64}
    errors::Array{Float64} = zeros(n)
    u::Int64 = -1
    Base.Threads.@threads for j in 1:lastindex(tb)
        u = tb[j][1]
        variance::Float64 = 0.0
        for i in 1:sample_size
            variance += (tilde_b[(i-1)*n+u] - (tb[j][2]/sample_size))^2
            #variance+= empirical_variance(tilde_b,sample_size,u)
        end
        variance = variance/(sample_size-1)
       # variance = variance
    ...
    converged::Bool = true
    overall_convergence::Int64 = 0
    for j in 1:lastindex(tb)
        u = tb[j][1]
        converged = converged & (errors[u] <= eps)
        if (errors[u] <= eps)
            overall_convergence+=1
        end
    end
    return converged,overall_convergence
end
