...
function threaded_progressive_rtb_shortest_foremost_bernstein(tg::temporal_graph,initial_sample::Int64,epsilon::Float64,delta::Float64,geo::Float64,verbose_step::Int64, bigint::Bool)::Tuple{Array{Float64},Array{Int64},Float64,Float64}
    ...
    while keep_sampling
        ...
        Base.Threads.@threads for i in 1:(sample_size_schedule[j]-sample_size_schedule[j-1])
            ...
            _ssftp_accumulate_bernstein!(tg,tal,tn_index,s,bigint,local_temporal_betweenness[Base.Threads.threadid()],t_bc[Base.Threads.threadid()])
        end
        ...
        println("P-RTB-SFM. Processed " * string(sampled_so_far) * " nodes in " * finish_partial * " seconds | Est. ξ = ",xi)
        if xi <= epsilon
            keep_sampling = false
        else
            j+=1
