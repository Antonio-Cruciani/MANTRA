...
function temporal_prefix_foremost_betweenness_multithread(tg::temporal_graph, verbose_step::Int64)::Tuple{Array{Float64},Float64}
    start_time = time()
    tal::Array{Array{Tuple{Int64,Int64}}} = temporal_adjacency_list(tg)
    temporal_prefix_foremost_betweenness_centrality::Array{Float64} = zeros(tg.num_nodes)
    ...
    Threads.@threads for s in 1:tg.num_nodes
        ...
        if (verbose_step > 0 && processed_so_far % verbose_step == 0)
            ...
            println("PREFIX. Processed " * string(processed_so_far) * "/" * string(tg.num_nodes) * " nodes in " * finish_partial * " seconds | Est. remaining time : "*time_to_finish)        end
    end
    finish_total::Float64 = round(time() - start_time; digits=4)
    return temporal_prefix_foremost_betweenness_centrality, finish_total
end